...
| `m6-release-candidate` | M0–M6 and the internal 0.4 product gate are accepted. The clean candidate passed 267 tests, 92% coverage, lint, strict typing, vulnerability/license audit, source/wheel build, oracle and benchmark verification, replay checks, and installed-wheel smoke on Python 3.11 and 3.12. This is the historical baseline inherited by the now-complete M7. | `docs/evidence/M6_RELEASE_VERIFICATION.md`; `docs/evidence/M6_SIGNOFF.md`; `docs/MILESTONE_6.md` | 2026-09-15 |
...
| `m7-entry` | Joshua Myers approved M7-D1 through M7-D3 without revision on 2026-09-15: stabilize the existing adapted surface for all 22 upstream exports, retain 29 gap clusters as post-1.0 candidates and reject 19, execute the locked robust-meta boundary plan before final disposition, and adopt the 1.x API/schema policy. These entry decisions are fulfilled by the completed M7. | `docs/MILESTONE_7.md`; `docs/m7/compatibility-disposition.json`; `docs/M7_CALIBRATION_PLAN.md`; `docs/adr/ADR-007-public-api-schema-stability.md` | 2026-09-15 |
| `m7a-public-contract` | Joshua Myers accepted M7A pass 2 on 2026-09-15. All 144 root exports remain retained without removals or renames. M7B superseded the all-stable candidate by marking seven dedicated robust-meta names experimental; the other 137 are stable 1.x names. | `docs/PUBLIC_API_REFERENCE.md`; `docs/MIGRATING_TO_1_0.md`; `docs/m7/public-contract.json`; `docs/evidence/M7A_PASS2_VERIFICATION.md` | 2026-09-15 |
...
| `m7c-stability` | M7C is complete. A 51-example corpus covers all 25 serialized public result types and 49 schema/discriminator triples; every example passes Draft 2020-12 validation plus field/type/version/discriminator/unknown-property mutations. Pre-1.0 schema gaps for grouped robust, composition limits, and Bayesian properties were corrected without changing emitted payloads. | `docs/m7/golden-results.json`; `tests/test_m7c_stability.py`; `docs/evidence/M7C_VERIFICATION.md` | 2026-09-15 |
| `m7d-release-candidate` | The 1.0.0 technical candidate has self-cleaning builds, machine-checked wheel/sdist contents and metadata, and offline installed-artifact smoke coverage on Python 3.11 and 3.12. The full check, audit, oracle, benchmark, build, and release-tag gates pass. Joshua Myers independently reviewed and accepted exact candidate `e682d9d` on 2026-09-15, closing M7 and the 1.0 product gate. | `docs/evidence/M7D_RELEASE_VERIFICATION.md`; `docs/evidence/M7D_INDEPENDENT_REVIEW.md`; `.github/workflows/ci.yml`; `.github/workflows/release.yml` | 2026-09-15 |
| `one-zero-release` | Annotated tag `v1.0.0` points to sign-off commit `bad099b`. Release workflow `35043264726` built once, passed all gates, attached the wheel, sdist, CycloneDX SBOM, and checksums, and published only the wheel/sdist through protected PyPI trusted publishing. PyPI filenames and SHA-256 values match the GitHub release, metadata is correct, neither file is yanked, and a no-cache Python 3.12 install passed the 144-export public smoke. | GitHub release `v1.0.0`; PyPI `plotsalot==1.0.0`; `docs/evidence/M7D_RELEASE_VERIFICATION.md` | 2026-09-15 |
