...
`plotsalot-dataset` publishes the reference market-observation CSV as an
...
```sh
uv run plotsalot-dataset publish data/example.csv data/processed \
  --dataset-version 2026-01-02.1 \
  --source-id fixture/example.csv \
  --revision "$(git rev-parse HEAD)" \
  --created-at-utc "$(date -u +%Y-%m-%dT%H:%M:%SZ)"

uv run plotsalot-dataset verify data/processed/2026-01-02.1
```
